...
class SpecificationCurve():
    ...
    def _spec_curve_regression(self):
        ...
        """
        df = self.df.copy()
        controls = copy.copy(self.controls)
        init_cols = self.y_endog + self.x_exog + controls
        exclu_grps = copy.copy(self.exclu_grps)
        df = df[init_cols]
        new_cols = []
        # Warning: hard-coded prefix
        if(self.cat_expand != []):
            df = pd.get_dummies(df, columns=self.cat_expand, prefix_sep=' = ')
            new_cols = [x for x in df.columns if x not in init_cols]
            # Now change the controls
            [controls.remove(x) for x in self.cat_expand]
            controls.extend(new_cols)
            # Create mapping from cat expand to new cols
...
# sc = SpecificationCurve(df, y_endog, x_exog, controls,
#                     cat_expand=['x_4'])
# sc = SpecificationCurve(df, y_endog, x_exog, controls)
# # The cat expand option does not work with new class
# # setup
# sc.fit()
# sc.plot()